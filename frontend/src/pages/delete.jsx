...
export default function Home() {
  ...
  const handleAdd = () => {
    ...
    if (!selectedLotteryType) {
      showWarningToast("Please select a lottery type");
      return;
    }
    if (!selectedShift) {
    ...
    if (!startNumber || !endNumber) {
      showWarningToast("Please enter both start and end numbers");
      return;
    }
    if (startNumber.length !== 5) {
      showWarningToast("Start number must be exactly 5 digits");
      return;
    }
 let adjustedEndNum = endNumber;
    ...
    if (countLottaryNumber > 2000) {
      showWarningToast("Maximum 2000 numbers allowed per entry");
      return;
    }
  ...
  const handleSubmitAll = async () => {
    if (pendingEntries.length === 0) {
      showWarningToast("No entries to submit");
      return;
    }
    ...
    try {
      for (const entry of pendingEntries) {
        ...
        // }
        try {
          const response = await fetchWithAuth("/seller/deleteNumberToSeller", {
            method: 'POST',
            body: JSON.stringify({
              sellerId: entry.sellerId,
              lotteryTypeId: entry.lotteryTypeId,
          ...
          showWarningToast(`❌ ${errorMsg} for range ${entry.startNumber} - ${entry.endNumber}`);
      ...
      showSuccessToast(`✅ ${pendingEntries.length} entries added successfully!`);
      setPendingEntries([]);
      setSelectedShift('');
